...
test.describe('Campaign Manager Suite | CX-Connect', () => {
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    // Retrieve credentials from environment variables (loaded from .env.uat)
    const userId   = process.env.USER_ID;
    const password = process.env.USER_PASSWORD;

    if (!userId || !password) {
      throw new Error('[Campaign Suite] USER_ID and USER_PASSWORD must be set in your .env.uat file');
    }

    console.log(`[Campaign Suite] Logging in once as: ${userId}`);
    ...
    console.log('[Campaign Suite] ✅ Login successful. Session will be reused for all campaign tests.');
